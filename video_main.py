...
def detectPose(image_pose):
    l_shldr,r_shldr,l_hip,r_hip = ((0,0),(0,0),(0,0),(0,0))
...
if __name__ == '__main__':
    ...
    # output = cv2.imread(image_path)
        ...
        image = cv2.line(image, rslt[0], rslt[2], (0,255,0), 2)
        image = cv2.line(image, rslt[1], rslt[3], (0,255,0), 2)
        
        shoulder_to_waist = findDistance(rslt[0],rslt[2])
        shirt_type = "medium"
        if (shoulder_to_waist < 105):
            shirt_type = "small"
        elif shoulder_to_waist > 130:
            shirt_type = "large"

        font = cv2.FONT_HERSHEY_SIMPLEX
        image = cv2.putText(image, f"Shoulder: {int(findDistance(rslt[0],rslt[1]))}cm, Waist: {int(findDistance(rslt[2],rslt[3]))}cm", (20,30), font, 
                        0.7, (0,255,255), 2, cv2.LINE_AA)
        image = cv2.putText(image, f"Shoulder_to_Waist: {int(shoulder_to_waist)}cm", (20,50), font, 
                        0.7, (0,255,255), 2, cv2.LINE_AA)
        image = cv2.putText(image, f"Shirt Type: {shirt_type}", (20,70), font, 
                        0.7, (0,255,255), 2, cv2.LINE_AA)
